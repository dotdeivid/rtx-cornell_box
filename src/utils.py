...
def obtener_punto_luz_aleatorio(centro_luz, radio_luz):
    """
    Genera un punto aleatorio dentro de una esfera de luz para simular área luminosa.

    En el mundo real, las luces no son puntuales perfectas sino que tienen volumen.
    Esta función simula una "luz de área esférica" generando puntos aleatorios
    dentro de un volumen esférico, lo que produce sombras suaves (penumbra).

    Algoritmo:
    1. Genera offset aleatorio en cada eje [-1, 1]
    2. Escala por el radio de la luz
    3. Desplaza el centro con este offset

    NOTA: Este es un método APROXIMADO (distribución cubica no esférica).
    Para distribución verdaderamente uniforme en una esfera, usar random_in_unit_sphere().

    ¿Por qué luces de área?
    - **Realismo**: Las bombillas reales tienen tamaño físico
    - **Sombras suaves**: Diferentes puntos de la luz crean penumbra
    - **Más natural**: Evita sombras duras irreales de luces puntuales

    Args:
        centro_luz (Vec3): Centro de la esfera de luz
        radio_luz (float): Radio de la esfera luminosa
                          Valores típicos:
                          - 0.0: Luz puntual (sombras duras)
                          - 0.1-0.5: Bombilla pequeña (penumbra sutil)
                          - 1.0+: Luz de área grande (penumbra amplia)

    Returns:
        Vec3: Punto aleatorio dentro del volumen esférico de la luz

    Efecto del radio:
        radio ↑ → Penumbra más suave y amplia
        radio ↓ → Sombras más definidas (tiende a puntual)
    ...
    Ejemplo:
        ...
        >>> radio = 0.3
        >>>
        >>> # 10 puntos de luz para sombras suaves
        >>> for _ in range(10):
        >>>     punto_luz = obtener_punto_luz_aleatorio(centro, radio)
        >>>     # Crear rayo de sombra hacia este punto
        >>>     shadow_ray = Ray(hit_point, punto_luz - hit_point)

    Distribución:
        Este método genera puntos en un CUBO [-radio, radio]³
        NO es distribución uniforme esférica (más puntos en esquinas).

        Para mayoría de casos prácticos, la diferencia es imperceptible.
        Para simulaciones físicamente precisas, usar random_in_unit_sphere().
    """
